...
@user.route('/register/isHaveEmail/', methods=['POST'])
def email_verify():
    '''
        函数功能：验证邮箱是否已注册
...
@user.route('/register/sendEmail/', methods=['POST'])
def send_email():
    '''
        函数功能：发送验证码
    ...
    if oldEmail: # 如果存在该邮箱未超时的话会发送与第一次请求相同的验证码
        if (datetime.now()-oldEmail.createTime).seconds>=1800: # 验证码已超时，使用新的验证码
            ...
            db.session.commit()
            content = "您正在咻一修网站上注册新用户，验证码为【{code}】。验证码30分钟内有效，请尽快完成注册，如非本人操作，请忽略。".format(code=verifyCode)
            result = sendMail(str(email), content)
        else:
            content = "您正在咻一修网站上注册新用户，验证码为【{code}】。验证码30分钟内有效，请尽快完成注册，如非本人操作，请忽略。".format(code=oldEmail.verifyCode)
            result = sendMail(str(email), content)
    else:  # 不存在该邮箱，则添加到数据库
...
def register():
    ...
    schoolCode = request.form.get('schoolCode')  # string
